...
export const filesystemAgent = createAgent({
  ...
  tools: [
...

    {
      id: 'readFile',
      description:
        'Reads the content of a file from the scratchpad directory',
      parameters: z.object({
    ...
    // Folder operations
    {
      ...
      parameters: z.object({
        folderPath: z
          .string()
          .describe('The name of the folder path to create'),
      }),
...

    {
      ...
      parameters: z.object({
        folderPath: z
          .string()
          .describe('The name of the folder path to remove'),
      }),
      execute: async ({ folderPath }) => {
        const fullPath = path.join(SCRATCHPAD_DIR, folderPath);
        try {
...

    {
      ...
      parameters: z.object({
        folderPath: z
          .string()
          .describe(
            'The folder path to retrieve the list of files and folders',
          ),
      }),
      execute: async ({ folderPath }) => {
        const fullPath = path.join(SCRATCHPAD_DIR, folderPath);
...

    {
      id: 'moveItem',
      description:
        'Moves a file or folder to a new location within scratchpad',
      parameters: z.object({
